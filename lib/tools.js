...
function checkUserName(str, minLen, maxLen) {
    ...
    minLen = minLen || ccs.config.UserNameMinLen;
    maxLen = maxLen || ccs.config.UserNameMaxLen;
...
function filterTitle(str) {
    ...
    str = trim(str, true);
    str = ccs.module.xss(str, options);
    return cutStr(str, ccs.config.TitleMaxLen, ccs.config.TitleMinLen);
}
...
function filterSummary(str) {
    ...
    str = ccs.module.xss(toStr(str), options);
    return cutStr(str, ccs.config.SummaryMaxLen);
}

function filterContent(str) {
    return cutStr(str, ccs.config.ContentMaxLen, ccs.config.ContentMinLen);
}
...
function checkTimeInterval(req, type, set) {
    return then(function (defer) {
        ccs.cache.timeInterval[set ? 'put' : 'get'](req.session._id + type, set ? 1 : defer, defer);
    });
}
