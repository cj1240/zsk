...
var client = {},
    ...
    redisPort = ccs.conf.RedisPort || 6379,
    redisIp = ccs.conf.RedisIp || '127.0.0.1',
    globalCacheDb = ccs.conf.RedisDefaultDb || 1,
    ...
    toNum = ccs.lib.tools.toNum,
    throttle = ccs.lib.tools.throttle,
...
function initConfig(configTpl, callback) {
    ...
    var update = throttle(function () {
        ccs.dao.index.setGlobalConfig(_config);
...
function onlineCache(req, callback) {
    ...
    if (key) {
        ...
        callback(ccs.Err('redis onlineCache request error!'));
    }
}
...
function userCache(_id, callback) {
    ...
    if (_id > 0) {
        then(function (defer) {
            ...
            if (!user) {
                callback(ccs.Err('redis user not exist!'));
            } else {
                user._id = +user._id;
                callback(null, user);
            }
        ...
        callback(ccs.Err('redis userCache request error!'));
    }
}
...
userCache.get = function (name, callback) {
    ...
    if (name) {
        then(function (defer) {
            ...
            callback(_id > 0 ? null : ccs.Err('redis user not exist!'), +_id);
        }).fail(function (defer, err) {
            callback(err);
        });
    } else {
        callback(ccs.Err('redis userCache.get request error!'));
    }
};
userCache.update = function (userObj, callback) {
    ...
    if (user._id > 0) {
        ...
        callback(ccs.Err('redis userCache update request error!'));
    }
};
...
function tagCache(_id, callback) {
    ...
    if (_id > 0) {
        then(function (defer) {
            ...
            if (!tag) {
                callback(ccs.Err('redis tag not exist!'));
            } else {
                tag._id = +tag._id;
        ...
        callback(ccs.Err('redis tagCache request error!'));
    }
}
...
tagCache.get = function (name, callback) {
    ...
    if (name) {
        then(function (defer) {
            ...
            callback(_id > 0 ? null : ccs.Err('redis tag not exist!'), +_id);
        }).fail(function (defer, err) {
            callback(err);
        });
    } else {
        callback(ccs.Err('redis tagCache.get request error!'));
    }
};
tagCache.update = function (tagObj, callback) {
    ...
    if (tag._id > 0) {
        ...
        callback(ccs.Err('redis tagCache update request error!'));
    }
};
...
function testCache(_id, callback) {
    callback = callback || callbackFn;

    if (_id > 0) {
        then(function (defer) {
            client[globalCacheDb].hgetall('test.hash.' + _id, defer);
        }).then(function (defer, test) {
                if (!test) {
                    callback(ccs.Err('redis test not exist!'));
                } else {
                    test._id = +test._id;
                    test.a = +test.a;
                    test.b = +test.b;
        ...
        callback(ccs.Err('redis testCache request error!'));
    }
}
testCache.removeAll = function (callback) {

    callback = callback || callbackFn;
    then(function (defer) {
        client[globalCacheDb].keys('test*', defer);
    }).then(function (defer, keys) {
            if (keys.length > 0) {
                client[globalCacheDb].del(keys, defer);
            } else {
...
testCache.update = function (tagObj, callback) {
    var tag = {
        _id: 0,
        a: '',
        b: '',
    ...
    callback = callback || callbackFn;
    intersect(test, tagObj);
    if (test._id > 0) {
        then(function (defer) {
            client[globalCacheDb].hgetall('test.hash.' + test._id, defer);
        }).all(function (defer, err, old) {
                if (old && test.test && old.test !== test.test) {
                    client[globalCacheDb].del(('test.str.' + old.test).toLowerCase(), defer);
                } else {
                    defer();
                }
            }).fail(function (defer, err) {
        ...
        callback(ccs.Err('redis testCache update request error!'));
    }
};
function articleCache(_id, callback) {
    ...
    if (_id > 0) {
        then(function (defer) {
            ...
            if (!article) {
                callback(ccs.Err('redis article not exist!'));
            } else {
        ...
        callback(ccs.Err('redis articleCache request error!'));
    }
}
...
articleCache.update = function (articleObj, callback) {
    ...
    if (article._id > 0) {
        ...
        callback(ccs.Err('redis articleCache update request error!'));
    }
};
