...
var msg = ccs.lib.msg,
    ...
    listCache = ccs.cache.list,
    articleDao = ccs.dao.article,
    articleCache = ccs.cache.article,
    commentCache = ccs.cache.comment,
...
articleCache.getP = function (ID, convert) {
    ...
    return then(function (defer) {
        if (ID >= 0) {
            ...
            defer(ccs.Err(msg.ARTICLE.articleNone));
        }
...
function calcuHots(article) {
    var hots = ccsConfig.ArticleHots;
...
function checkStatus(article) {
    var status = ccsConfig.ArticleStatus,
...
function filterArticle(articleObj) {
    return then(function (defer) {
        ...
        if (!(newObj.title = filterTitle(newObj.title))) {
            return defer(ccs.Err(msg.ARTICLE.titleMinErr));
        }
        if (!(newObj.content = filterContent(newObj.content))) {
            return defer(ccs.Err(msg.ARTICLE.articleMinErr));
        }
        ...
        if (newObj.tagsList && newObj.tagsList.length > 0) {
            tagAPI.filterTags(newObj.tagsList.slice(0, ccsConfig.ArticleTagsMax)).then(function (defer2, tagsList) {
                if (tagsList) {
...
function addComment(req, ID) {
    ...
    return then(function (defer) {
        checkTimeInterval(req, 'AddComment').all(function (defer2, err, value) {
            if (value) {
                defer(ccs.Err(msg.MAIN.timeIntervalErr + '[' + ccsConfig.TimeInterval + 's]'));
            } else {
...
function setMark(req, ID) {
    ...
    return then(function (defer) {
        ...
        if (mark && index >= 0) {
            defer(ccs.Err(msg.USER.userMarked));
        } else if (!mark && index < 0) {
            defer(ccs.Err(msg.USER.userUnmarked));
        } else {
            articleDao.setMark({
                _id: ID,
                markList: mark ? req.session.Uid : -req.session.Uid
            });
            ccs.dao.user.setMark({
                _id: req.session.Uid,
                markList: mark ? ID : -ID
            });
            if (mark) {
...
function setFavor(req, ID) {
    ...
    return then(function (defer) {
        ...
        if (favor && index >= 0) {
            defer(ccs.Err(msg.USER.userFavor));
        } else if (!favor && index < 0) {
            defer(ccs.Err(msg.USER.userUnoppose));
        } else {
            articleDao.setFavor({
                _id: ID,
...
function setOppose(req, ID) {
    ...
    return then(function (defer) {
        ...
        if (oppose && index >= 0) {
            defer(ccs.Err(msg.USER.userOppose));
        } else if (!oppose && index < 0) {
            defer(ccs.Err(msg.USER.userUnfavor));
        } else {
            articleDao.setOppose({
                _id: ID,
...
function getArticle(req, res) {
    getArticleID(req).then(function (defer, article) {
        if (article.display > 0 && !req.session.Uid) {
            ...
            defer(ccs.Err(msg.ARTICLE.articleDisplay1));
        } else if (article.display === 2 && req.session.role !== 5) {
            defer(ccs.Err(msg.ARTICLE.articleDisplay2));
        } else {
            articleCache.getP(article._id).all(defer);
        }
    }).then(function (defer, article) {
...
function setArticle(req, res) {
    ...
    getArticleID(req).then(function (defer, article) {
        if (req.session.role < 1) {
            defer(ccs.Err(msg.USER.userRole0));
...
function robot(req, res) {
    ...
    then(function (defer) {
        obj.global = intersect(union(configPublicTpl), ccsConfig);
        ...
        if (checkID(ID, 'A')) {
            ...
            then(function (defer2) {
                ...
                article.content = ccs.module.marked(article.content);
                ...
                each(commentsList, function (comment, i, list) {
                    list[i].content = ccs.module.marked(comment.content);
                });
        ...
        then(function (defer2) {
            ...
            keywords = ccs.lib.tools.uniqueArray(keywords);
            ...
            each(list, function (article, i) {
                list[i].content = ccs.module.marked(article.content);
...
function sitemap(req, res) {
    ...
    function toSiteMap(article) {
        return {
            url: ccsConfig.url + '/' + convertArticleID(article._id),
