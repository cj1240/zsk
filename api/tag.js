...
var msg = ccs.lib.msg,
    ...
    filterTag = ccs.lib.tools.filterTag,
    ...
    tagCache = ccs.cache.tag,
    cache = ccs.lib.redis.tagCache,
...
function setTag(tagObj) {
    ...
    return then(function (defer) {
                ...
                tagCache.getP(tagObj._id).then(function (defer3, tag) {
                    then.each(tag.articlesList, function (defer4, x) {
                        if (x) {
                            ...
                            ccs.cache.list.getP(x, false).then(function (defer5, article) {
                                removeItem(article.tagsList, tagObj._id);
                                if (article.tagsList.indexOf(toID) < 0) {
                                    article.tagsList.push(toID);
                                    ccs.cache.list.put(article._id, article);
                                    ccs.cache.article.update(article._id, function (value) {
                                        value.tagsList = article.tagsList;
                                        return value;
                                    });
                                    ccs.dao.article.setArticle({
                                        _id: x,
                        ...
                        if (x) {
                            ...
                            ccs.cache.user.getP(x, false).then(function (defer5, user) {
                                removeItem(user.tagsList, tagObj._id);
                                if (user.tagsList.indexOf(toID) < 0) {
                                    user.tagsList.push(toID);
                                    ccs.cache.user.put(user._id, user);
                                    ccs.dao.user.setUserInfo({
                                        _id: x,
        ...
        defer(err === true ? ccs.Err(msg.MAIN.requestDataErr) : err);
    });
}
    ...
    return then(function (defer) {
        if (checkID(tag, 'T')) {
            ...
            cache.get(tag, function (err, ID) {
                defer(err ? ccs.Err(msg.TAG.tagNone) : null, ID);
            });
        }
