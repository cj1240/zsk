...
var msg = ccs.lib.msg,
    ...
    tagAPI = ccs.api.tag,
    redis = ccs.lib.redis,
    userDao = ccs.dao.user,
    userCache = ccs.cache.user,
...
userCache.getP = function (Uid, convert) {
    ...
    return then(function (defer) {
        if (Uid >= 0) {
            ...
            defer(ccs.Err(msg.USER.UidNone));
        }
...
function calcuScore(user) {
    ...
    var UsersScore = ccs.config.UsersScore;
    user.score = 0;
...
function adduser(userObj) {
    return then(function (defer) {
        if (typeof userObj !== 'object') {
            defer(ccs.Err(msg.USER.userNone));
        } else if (!checkEmail(userObj.email)) {
            defer(ccs.Err(msg.USER.userEmailErr));
        } else if (!checkUserName(userObj.name)) {
            defer(ccs.Err(msg.USER.userNameErr));
        } else {
            defer();
        }
    }).then(function (defer) {
        cache.get(userObj.email, function (err, Uid) {
            if (Uid) {
                defer(ccs.Err(msg.USER.userEmailExist));
            } else {
                defer();
            }
        });
        ...
        cache.get(userObj.name, function (err, Uid) {
            if (Uid) {
                defer(ccs.Err(msg.USER.userNameExist));
            } else {
                defer();
            }
        });
        ...
        userObj.role = ccs.config.emailVerification ? 1 : 2;
...
function setReset(resetObj) {
    ...
    return then(function (defer) {
        ...
        resetUrl = ccs.config.url + '/reset?req=' + resetUrl;
        var email = resetObj.e || doc.email;
        return ccs.lib.email.tpl(ccs.config.title, doc.name, email, resetUrl, resetObj.r).send(defer);
    }).fail(errorHandler);
}
...
function userLogin(loginObj) {
    ...
    return then(function (defer) {
        if (typeof loginObj !== 'object') {
            defer(ccs.Err(msg.MAIN.requestDataErr));
        } else if (date - loginObj.logtime > 259200000) {
            defer(ccs.Err(msg.MAIN.requestOutdate));
            ...
            cache.get(loginObj.logname, function (err, Uid) {
                if (Uid >= 0) {
                    ...
                    if (checkEmail(loginObj.logname)) {
                        defer(ccs.Err(msg.USER.userEmailNone));
                    } else if (checkUserName(loginObj.logname)) {
                        defer(ccs.Err(msg.USER.userNameNone));
                    } else {
                        defer(ccs.Err(msg.USER.logNameErr));
                    }
                }
            });
        }
        ...
        userDao.getAuth(Uid, function (err, user) {
            if (!user) {
                defer(ccs.Err(msg.MAIN.dbErr));
            } else if (user.locked) {
                defer(ccs.Err(msg.USER.userLocked, 'locked'));
                ...
                defer(ccs.Err(msg.USER.loginAttempts));
                ...
                return defer(ccs.Err(msg.USER.userPasswd, 'passwd'));
            }
        });
    }).fail(errorHandler);
}
...
function register(req, res) {
    ...
    function emailToAdmin(doc) {
        if (ccs.config.email) {
            var url = ccs.config.url + '/' + doc._id;
            ccs.lib.email.tpl(ccs.config.title, doc.name, ccs.config.email, url, 'register').send();
        }
    }

    then(function (defer) {
        if (!ccs.config.register) {
            defer(ccs.Err(msg.MAIN.registerClose));
        } else {
            checkTimeInterval(req, 'Register').all(function (defer2, err, value) {
                if (value) {
                    defer(ccs.Err(msg.MAIN.timeIntervalErr + '[' + ccs.config.TimeInterval + 's]'));
                } else {
                    defer();
                }
            });
        ...
        if (ccs.config.emailVerification) {
            setReset({
                u: req.session.Uid,
                r: 'role'
            }).then(function () {
            ...
            if (!list || p === 1) {
                if (req.path[3] === 'fans') {
                    ...
                    then.each(user.articlesList.reverse(), function (defer3, ID) {
                        ...
                        paginationCache.put(key, list);
                        defer2(null, list, ccs.cache.list);
                    });
                }
            } else {
                defer2(null, list, req.path[3] === 'fans' ? userCache : ccs.cache.list);
            }
...
function setUser(req, res) {
    getUserID(req).then(function (defer, Uid) {
        if (!req.session.Uid) {
            defer(ccs.Err(msg.USER.userNeedLogin));
        ...
        userCache.getP(req.session.Uid, false).then(function (defer2, user) {
            if (follow && user.followList.indexOf(Uid) >= 0) {
                return defer(ccs.Err(msg.USER.userFollowed));
            } else if (!follow && user.followList.indexOf(Uid) < 0) {
                return defer(ccs.Err(msg.USER.userUnfollowed));
            } else {
                userDao.setFollow({
                    _id: req.session.Uid,
    ...
    then(function (defer) {
        ...
        if (!list || p === 1) {
            then(function (defer2) {
                ...
                ccs.cache.list.getP(x, false).all(function (defer3, err, article) {
        ...
        paginationList(req, list, ccs.cache.list, defer);
    }).then(function (defer, data, pagination) {
        var now = Date.now(),
...
function editUser(req, res) {
    ...
    then(function (defer) {
        ...
        if (userObj.tagsList) {
            then(function (defer2) {
                tagAPI.filterTags(userObj.tagsList.slice(0, ccs.config.UserTagsMax)).all(defer2);
            }).then(function (defer2, tagsList) {
...
function getReset(req, res) {
    ...
    then(function (defer) {
        if (!resetObj.r || ['locked', 'email', 'passwd', 'role'].indexOf(resetObj.r) === -1) {
            defer(ccs.Err(msg.MAIN.resetInvalid));
            ...
            then(function (defer2) {
                if (checkUserID(req.apibody.name)) {
                    ...
                    cache.get(req.apibody.name, function (err, Uid) {
                        defer2(Uid ? null : ccs.Err(msg.USER.userEmailExist), Uid);
                    });
                } else {
                    defer2(ccs.Err(msg.USER.userNameNone));
                }
            }).then(function (defer2, Uid) {
                cache(Uid, function (err, user) {
                    defer2(user ? null : ccs.Err(msg.USER.UidNone), user);
                });
            }).then(function (defer2, user) {
                resetObj.u = user._id;
                resetObj.e = user.email;
                if (req.apibody.email.toLowerCase() !== resetObj.e.toLowerCase()) {
                    defer2(ccs.Err(msg.USER.userEmailNotMatch));
                } else {
                    defer();
                }
            }).fail(defer);
...
function resetUser(req, res) {
    ...
    then(function (defer) {
        ...
        if (user && user.resetKey && (now - user.resetDate) / 86400000 < 1) {
            ...
            defer(ccs.Err(msg.MAIN.resetOutdate));
        }
    }).then(function (defer, user) {
        setCache(user);
        ...
        if (!list || p === 1) {
            userCache.getP(req.session.Uid, false).then(function (defer2, user) {
                ...
                then.each(user.articlesList, function (defer3, x) {
                    ccs.cache.list.getP(x, false).then(function (defer4, article) {
                        if (article) {
                            if (article.status > -1) {
                                articlesList.push(x);
                            } else {
